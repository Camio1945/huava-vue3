  ...
  <div class="material" v-loading="pager.loading">
    ...
    <div class="material__center flex flex-col">
          ...
          >
            ...
            <div>
              ...
              <el-select v-model="moveId" placeholder="请选择">
                <template v-for="item in catePage" :key="item.id">
      ...
      <div class="material-center__content flex flex-col flex-1 mb-1 min-h-0">
        <el-scrollbar v-if="pager.list.length" v-show="listShowType == 'normal'">
          <ul class="file-list flex flex-wrap mt-4">
            <li class="file-item-wrap" v-for="item in pager.list" :key="item.id" :style="{ width: fileSize }">
              ...
              <div class="operation-btns flex items-center">
                ...
                <el-link v-else type="primary" :underline="false" style="margin-left: 25px" :href="item.url"
                  >下载
                </el-link>
              </div>
            </li>
        ...
        >
          ...
          <el-table-column label="操作" width="150" fixed="right">
            <template #default="{ row }">
              ...
              <div class="inline-block">
                <el-button type="primary" link @click.stop="handlePreview(row.url)"> 查看 </el-button>
              </div>
              <div class="inline-block">
        ...
        <div class="flex flex-1 justify-center items-center" v-if="!pager.loading && !pager.list.length">暂无数据~</div>
      </div>
    ...
    <div class="material__right" v-if="mode == 'picker'">
      ...
      <div class="flex-1 min-h-0">
        <el-scrollbar class="ls-scrollbar">
          <ul class="select-list flex flex-col p-t-3">
...
watch(
  ...
  (val: any[]) => {
    emit('change', val)
    if (val.length == pager.list.length && val.length !== 0) {
