...
export function useCate(type: number) {
  ...
  // 分组列表
  const catePage = ref<any[]>([])

  // 选中的分组id
  const cateId = ref<number | string>('')

  // 获取分组列表
  const getCatePage = async () => {
    const data = await fileCatePage({
      page_type: 0,
      type
    })
    const item: any[] = [
    ...
    catePage.value = data.list
    catePage.value.unshift(...item)
    setTimeout(() => {
...
export function useFile(cateId: Ref<string | number>, type: Ref<number>, limit: Ref<number>, size: number) {
  ...
  const { pager, getPage, resetPage } = usePaging({
    fetchFun: fileList,
    params: fileParams,
    firstLoading: true,
    size
